...
and VmOpCode = {
    ...
    override this.ToString() =
        let bytes = BitConverter.ToString(this.Buffer).Replace("-", String.Empty).PadRight(12)
...
and IrOpCode(opType: IrInstruction, useMultipleOpcodeForSameInstruction: Boolean) =
    ...
    member this.Assemble(ip: Int32, symbolTable: SymbolTable) =
        ...
        let (opBytes, vmOpCode) =
            ...
            | JumpIfGreater -> getVmOpCode(VmJumpIfGreater)
            | JumpIfGreaterEquals -> getVmOpCode(VmJumpIfGreaterEquals)
